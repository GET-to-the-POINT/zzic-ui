	...
	<div class="relative {sizeClasses[size]}">
		{#each icons as IconComponent, i}
			{#if i === currentIcon}
				...
				>
					<IconComponent class="animate-bounce" size={size === 'small' ? 32 : size === 'medium' ? 48 : 64} />
				</div>
			{/if}
		{/each}
		
		<!-- 회전하는 링 -->
		<div class="absolute inset-0 border-4 border-pink-200 border-t-pink-500 rounded-full animate-spin"></div>
	</div>
