...
<script>
  ...
  // form action 결정
  const formAction = mode === 'edit' && todoResponse 
    ? `/todos/${todoResponse.id}?/update` 
    : '/todos';
</script>
    ...
    {#if mode === 'edit' && todoResponse}
      <div class="flex gap-2 pt-2 border-t border-surface-200-800 mt-4">
        ...
        <form action={`/todos/${todoResponse.id}?/update`} method="POST" use:enhance class="flex-1">
          <button
            type="submit"
            class="btn w-full {todoResponse.statusId === 1 ? 'preset-tonal-success' : 'preset-tonal'}"
            name="statusId"
