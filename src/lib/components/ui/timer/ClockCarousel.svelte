...
>
	...
	<div
		...
		style="transform: translateX(calc(-{currentClockType * 100}% + {isDragging
			? dragOffset
			: 0}px))"
	>
		<!-- 디지털 시계 -->
		<div class="w-full flex-shrink-0 flex items-center justify-center">
			<DigitalClock {time} {colonVisible} size="text-7xl" />
		</div>

		<!-- 프로그레스 시계 -->
		<div class="w-full flex-shrink-0 flex items-center justify-center">
			<ProgressClock {time} {colonVisible} {progress} {progressColor} size={280} />
		</div>
	</div>
	...
	<div class="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
		{#each clockTypes as type, i}
			<button
				class="w-2 h-2 rounded-full transition-all {i === currentClockType
					? 'w-6 bg-primary-500'
					: 'bg-surface-400'}"
