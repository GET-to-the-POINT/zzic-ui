...
<!-- 필 라이트 (보조 조명) -->
<T.DirectionalLight position={[-5, 8, 6]} intensity={0.6} color="#f0e6ff" />

<!-- 백 라이트 (윤곽 조명) -->
<T.DirectionalLight position={[0, -3, -12]} intensity={0.9} color="#ffe6f0" />

<!-- 환경광 -->
<T.AmbientLight intensity={0.3} color="#fdf2f8" />

<!-- 스페큘러 하이라이트용 포인트 라이트들 -->
<T.PointLight position={[4, 5, 3]} intensity={3.0} color="#ffffff" distance={12} decay={2} />
<T.PointLight position={[-4, 4, 3]} intensity={2.5} color="#fff5f8" distance={10} decay={2} />
<T.PointLight position={[0, -2, 6]} intensity={1.5} color="#f8f0ff" distance={8} decay={2} />
